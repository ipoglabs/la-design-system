...
| File | Responsibility |
|---|---|
| `middleware.ts` | Read `cf-ipcountry` (env-configurable header name) → set cookie or redirect |
...
| File | Responsibility |
|---|---|
| `middleware.ts` | No header → set `countryPending=1` → pass through |
...
2. `middleware.ts` — route guard + Option A header read path
...
6. `app/country-select/page.tsx` — standalone fallback page (for middleware redirect)
